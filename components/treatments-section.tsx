  ...
  return (
    <section id="treatments" className="treatments-editorial relative scroll-mt-24 bg-background py-24 lg:py-36" aria-labelledby="treatments-title">
      <div className="mx-auto max-w-7xl px-6 lg:px-10">
        ...
        <div className="treatments-layout mt-16 grid gap-14 lg:mt-24 lg:grid-cols-[minmax(17rem,0.8fr)_minmax(0,1.2fr)] lg:gap-20">
          ...
          <article className="treatment-detail" aria-live="polite">
            ...
            <div className="grid gap-8 md:grid-cols-[minmax(0,0.8fr)_minmax(18rem,1.2fr)] md:items-start md:gap-10">
              <div className="order-2 md:order-1">
                ...
                <div className="mt-8"><p className="kicker text-foreground">Helps with</p><ul className="mt-4 grid gap-2 text-sm text-muted-foreground">{treatment.helpsWith.map((item) => <li key={item} className="flex items-baseline gap-3"><span className="h-px w-4 bg-primary" />{item}</li>)}</ul></div>
                <div className="mt-8 flex flex-wrap gap-2">{treatment.tags.map((tag) => <span key={tag} className="rounded-full bg-muted px-3 py-1.5 text-xs text-secondary-foreground">{tag}</span>)}</div>
              </div>
              <figure className="treatment-image order-1 overflow-hidden rounded-sm bg-muted md:order-2">
